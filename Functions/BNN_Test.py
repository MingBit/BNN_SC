...
x = Cont_Top2000.values #returns a numpy array
min_max_scaler = preprocessing.MinMaxScaler()
x_scaled = min_max_scaler.fit_transform(x)
Norm_Cont_Top2000 = pd.DataFrame(x_scaled)


#np.savetxt(path + 'HSC_geneSymbol')

#check expression distribution
#ESC_Data = ESC_Data.astype(int)
#tmp = ESC_Data.T.plot.hist(xlim = [0, 20], legend = False)

#HSC_Data = HSC_Data.astype(int)
#HSC_Data.plot.hist(legend = False)
...
def x_y_generator(Expr_df, gene, test_size = .6):
	
	X = Expr_df.drop(columns = gene)
	Y = Expr_df[gene]
...
#	print(model['out'].tag.test_value)
	...
	with model:
		...
		#generate 5,000 samples from the posterior distribution
...
def get_BNN_table(input_dataset, output_path, filename, hidden_layers):
	...
	BNN_Expr_data = pd.DataFrame()	
		
	for gene in input_dataset.columns:
		
		X_train, X_test, Y_train, Y_test = x_y_generator(input_dataset, gene)
		gene_train, gene_test = pred_eva(x_train= X_train, x_test=X_test, y_train= Y_train, hidden_layers = hidden_layers,
									 y_test=Y_test, bnn_func=construct_bnn, input_nodes = X_train.shape[1])
		
		if len(BNN_Expr_data.index) == 0:
...
BNN_Expr_data = get_BNN_table(input_dataset= ESC_Data, output_path= '/tmp/', 
							  filename= 'ESC_Test', hidden_layers=[50, 20])
